...
# Compatible with the GSA-specific mass-conserving LPUE workflow.
...
#'   numeric effort columns identified by grid-cell names. Optional `YEAR`,
#'   `GSA_code`, `GSA_LB` and `Species` columns are treated as identifiers rather
#'   than as effort. The main workflow nevertheless calls the function on one
#'   GSA/species/gear/month stratum at a time.
...
getLPUE <- function(xy,
  ...
  identifier_columns <- intersect(
    c("CFR", "YEAR", "MONTH", "GSA_code", "GSA_LB", "Species", "Gear"),
    names(xy)
  )
...
attr(getLPUE, "smart31_version") <- "2026-08-11"
